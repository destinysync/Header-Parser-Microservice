	...
	function headerParser(headerObj) {
			var ip = headerObj["x-forwarded-for"];
			var acceptedLang = headerObj["accept-language"].match(/.+(?=,)/)[0];
			var os = headerObj["user-agent"].match(/\((.+)\)/i)[1];
			var headerParserJSON = {
				'ipaddress': ip,
				'language': acceptedLang,
				'software': os
			}; 
			return headerParserJSON;
	}
	app.get('/*', function(req, res) {
		...
		if (req.originalUrl.length == 1) {
			res.json(headerParser(req.headers));
		} else if (urlPath == 'api/whoami/' || urlPath == 'api/whoami' ) {
			res.json(headerParser(req.headers));
		}
		else {
